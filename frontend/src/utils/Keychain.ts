...
export class Keychain {
  ...
  static async init(password: string): Promise<Keychain> {
    ...
    const masterHmac = await subtle.importKey(
      'raw',
      master,
      { name: 'HMAC', hash: 'SHA-256' },
      false,
    ...
    const hmacKeyBytes = new Uint8Array(
      await subtle.sign('HMAC', masterHmac, Keychain._strToBuf('hmac-key')),
    ).slice(0, 32)
    const aesKeyBytes = new Uint8Array(
      await subtle.sign('HMAC', masterHmac, Keychain._strToBuf('aes-key')),
    ).slice(0, 32)
  ...
  private async _encrypt(plain: Uint8Array, aadB64: string) {
    ...
    return {
      iv: Keychain._b64Encode(iv),
      ct: Keychain._b64Encode(new Uint8Array(ct)),
    }
  }
